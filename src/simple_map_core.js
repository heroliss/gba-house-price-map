...
<main class="app">
  <section class="mapShell">
    ...
    <div class="presetBar">${(config.viewPresets || []).map(preset => `<button class="presetButton" data-preset="${esc(preset.id)}">${esc(preset.label)}</button>`).join("")}</div>
...
function addDetailDataset(source,payload){if(document.getElementById('detail-'+source.id))return;const group=document.createElementNS('http://www.w3.org/2000/svg','g');group.id='detail-'+source.id;group.classList.add('detailDataset');group.dataset.minScale=String(source.minScale||3);group.innerHTML='<g>'+payload.regions.map(r=>'<path class="regionSeal" d="'+r.d+'" fill="'+r.fill+'" stroke="'+r.fill+'"></path>').join('')+'</g><g>'+payload.regions.map(r=>'<path id="'+r.id+'" class="region detailRegion" data-city="'+r.city+'" data-name="'+r.name+'" d="'+r.d+'" fill="'+r.fill+'"></path>').join('')+'</g>';detailLayer.appendChild(group);const boundary=document.createElementNS('http://www.w3.org/2000/svg','g');boundary.id='detail-boundary-'+source.id;boundary.classList.add('detailDataset');boundary.dataset.minScale=String(source.minScale||3);boundary.innerHTML=payload.outlines.map(r=>'<path class="cityBoundary" d="'+r.d+'"></path>').join('');detailBoundaries.appendChild(boundary);const labels=document.createElementNS('http://www.w3.org/2000/svg','g');labels.id='detail-label-'+source.id;labels.classList.add('detailDataset');labels.dataset.minScale=String(source.minScale||3);labels.innerHTML=payload.regions.filter(r=>r.price).map(r=>'<text class="detailLabel" x="'+r.cx+'" y="'+r.cy+'" data-min-scale="'+Math.max(r.labelMinScale,source.labelScale||3.2)+'" text-anchor="middle">'+r.label+' '+fmtWan(r.price)+'</text>').join('');dynamicLabels.appendChild(labels);payload.regions.forEach(r=>{places.push(r);placeById.set(r.id,r)});const newRegionEls=Array.from(group.querySelectorAll('.region'));newRegionEls.forEach(bindRegionElement);regionEls=regionEls.concat(newRegionEls);registerDetailLabels(labels);updateDetailVisibility();renderRows(search.value);applyTransform({forceLabels:true})}
...
function updateDetailVisibility(){Array.from(document.querySelectorAll('.detailDataset')).forEach(el=>el.classList.toggle('hidden',state.scale<Number(el.dataset.minScale||3)))}
function maybeLoadDetails(){if(!detailSources.length)return;const bounds=visibleContentBounds();detailSources.forEach(source=>{if(state.scale>=source.minScale&&(!source.bounds||intersects(bounds,source.bounds)))loadDetailSource(source)})}
