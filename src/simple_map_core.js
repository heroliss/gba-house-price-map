...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  @media (max-width:760px){ .app{padding:6px;gap:8px;max-width:100vw;overflow:hidden}.mapShell{min-height:82svh;border-radius:10px}header{left:8px;top:8px;max-width:calc(100% - 146px);padding:7px 9px;border:1px solid rgba(219,230,227,.9);border-radius:10px;background:rgba(255,255,255,.72);backdrop-filter:blur(8px)}h1{font-size:17px;line-height:1.1}header p{display:none}.toolbar{left:auto;right:8px;top:8px;width:121px;display:grid;grid-template-columns:repeat(2,58px);gap:5px}.toolbar button,.toolbar .mapNav{width:58px;height:32px;padding:7px 0;font-size:12px;text-align:center;justify-content:center}.toolbar #toggleTiles{width:121px;grid-column:1/3}.overlayControl{left:8px;right:8px;top:auto;bottom:80px;width:auto;padding:6px 8px;font-size:11px}svg{height:82svh}.cityLabel{font-size:calc(17px * var(--label-scale));stroke-width:4.8px}.detailLabel{font-size:calc(11px * var(--label-scale));stroke-width:2.9px}.legend{left:8px;right:8px;bottom:8px;padding:8px;border-radius:10px}.swatch{width:auto;flex:1;min-width:0;height:12px}.legendLabels{grid-template-columns:repeat(${colors.length},1fr);gap:4px;font-size:8.4px}.side{padding:14px;border-radius:10px}.sortBar{grid-template-columns:repeat(2,1fr)} }
</style>
</head>
<body>
<main class="app">
  <section class="mapShell">
    ...
    <svg id="map" viewBox="${mapViewBox.x.toFixed(2)} ${mapViewBox.y.toFixed(2)} ${mapViewBox.w.toFixed(2)} ${mapViewBox.h.toFixed(2)}" preserveAspectRatio="xMidYMid meet">
      <g id="viewport">
        ...
        <g id="priceOverlay"><g id="regionSeals">${regions.map(r => `<path class="regionSeal" d="${r.d}" fill="${r.fill}" stroke="${r.fill}"></path>`).join("")}</g><g id="regionLayer">${regions.map(r => `<path id="${r.id}" class="region" data-city="${esc(r.city)}" data-name="${esc(r.name)}" d="${r.d}" fill="${r.fill}"></path>`).join("")}</g></g>
        ...
        <g id="detailLabels">${regions.filter(r => r.price).map(r => `<text class="detailLabel" x="${r.cx}" y="${r.cy}" data-min-scale="${r.labelMinScale}" text-anchor="middle">${esc(r.label)} ${fmtWanLabel(r.price)}</text>`).join("")}</g>
      </g>
    </svg>
...
const labelEls=Array.from(document.querySelectorAll('.detailLabel,.cityLabel')).map(el=>({el,x:Number(el.getAttribute('x')),y:Number(el.getAttribute('y'))}));
const detailLabelEls=Array.from(document.querySelectorAll('.detailLabel')).map(el=>({el,minScale:Number(el.dataset.minScale||2.2)}));
const sortButtons=Array.from(document.querySelectorAll('.sortBar button')),collator=new Intl.Collator('zh-Hans-CN');
let state={scale:1,x:0,y:0},dragging=false,last=null,activePointers=new Map(),pinch=null,tapCandidate=null,activeId=null,labelsVisible=true,sortMode='priceDesc',tilesVisible=false,overlayOpacity=Number(overlayOpacityInput.value)/100,labelScale=Number(labelScaleInput.value)/100,lastTileKey='',pendingTileKey='',tileLoadToken=0,activeTileBuffer=0,rowEls=[],renderFrame=0,renderForceLabels=false,renderImmediateTiles=false,lastRenderedScale=0,tileUpdateTimer=0,tooltipPlaceId='';
const minScale=.7,maxScale=12,projection=${JSON.stringify({ minX: projection.minX, minY: projection.minY, maxX: projection.maxX, maxY: projection.maxY, scale: projection.scale, ox: projection.ox, oy: projection.oy })},mapBounds=${JSON.stringify(projection.projectedMapBounds)};
...
regionEls.forEach(el=>{const r=placeById.get(el.id);el.addEventListener('mousemove',event=>{setHoverHighlight(r);showTip(event,r)});el.addEventListener('mouseleave',()=>{setHoverHighlight(null);hideTip()})});
...
function renderRows(filter=''){const q=filter.trim().toLowerCase(),main=sortPlaces(places.filter(r=>r.price).filter(r=>!q||(r.city+r.name).toLowerCase().includes(q)));rows.innerHTML=main.map(r=>'<div class="row" data-id="'+r.id+'"><b>'+r.city+' '+r.name+'</b><span class="price">'+fmt(r.price)+'</span><span class="'+(String(r.mom).startsWith('+')?'up':String(r.mom).startsWith('-')?'down':'')+'">'+r.mom+'</span></div>').join('');rowEls=Array.from(rows.querySelectorAll('.row'));rowEls.forEach(row=>row.onclick=()=>selectRegion(row.dataset.id,true));regionEls.forEach(el=>{const r=placeById.get(el.id),match=!q||(r.city+r.name).toLowerCase().includes(q);el.classList.toggle('dimmed',!match)})} sortButtons.forEach(button=>button.addEventListener('click',()=>{sortMode=button.dataset.sort;sortButtons.forEach(item=>item.classList.toggle('active',item===button));renderRows(search.value)}));search.addEventListener('input',()=>renderRows(search.value));renderRows();applyOverlayOpacity();applyLabelScale();applyTransform({forceLabels:true,immediateTiles:true});
</script>
</body>
</html>`;
}
